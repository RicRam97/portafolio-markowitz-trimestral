...
def train_and_predict_models(supabase: Client, tickers: list[str]) -> dict:
    ...
    for ticker in tickers:
        try:
            ...
            res = supabase.table("activos_precios")\
            ...
            data = res.data
            if len(data) < 100:
                log.warning(f"Insuficientes datos para entrenar {ticker} ({len(data)} filas). Skipeando.")
                stats["failed"] += 1
                continue
            ...
            df_train = df_feats.dropna(subset=['ret_1d', 'ret_5d', 'ret_21d', 'vol_21d', 'dist_sma_20', 'target_21d_fwd']).copy()
            
            if len(df_train) < 50:
                log.warning(f"Muy pocos datos válidos tras feature eng para {ticker}. Skipeando.")
                stats["failed"] += 1
                continue
            ...
            stats["success"] += 1
            log.info(f"[{ticker}] Predicción guardada: {pred_ensamble*100:.2f}% (21-días forward).")
